...
type Msg = 
 ...
 CustomButtonSubmit
type RandomResult = None | Success | Failure | Error
 ...
 case msg of
  ...
  CustomButtonSubmit ->
   case model.submitButton of
    Allowed f -> ( model, generateRandom f)
    Disallowed -> ( { model | result = Error }, Cmd.none )
...
getResultDiv model =
 case model.result of
  Success -> div [ style "color" "green" ] [ text "SUCCESS!" ]
  Failure -> div [ style "color" "red" ] [ text "FAILURE!" ]
  None -> div [ style "color" "black" ] [ text "Click a button"]
